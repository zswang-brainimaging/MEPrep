...
def main():
    ...
    #    sentry_sdk.add_breadcrumb(message='fMRIPrep started', level='info')
        sentry_sdk.add_breadcrumb(message='MEPrep started', level='info')  ##ZSW
    #     sentry_sdk.capture_message('fMRIPrep started', level='info')
        sentry_sdk.capture_message('MEPrep started', level='info')   ##ZSW
 ...
 ##   config.loggers.workflow.log(25, 'fMRIPrep started!')
    config.loggers.workflow.log(25, 'MEPrep started!')   ##ZSW
    ...
    #    config.loggers.workflow.critical('fMRIPrep failed: %s', e)
        config.loggers.workflow.critical('MEPrep failed: %s', e)  ##ZSW
        raise
    else:
   ##     config.loggers.workflow.log(25, 'fMRIPrep finished successfully!')
        config.loggers.workflow.log(25, 'MEPrep finished successfully!')  ##ZSW
        if sentry_sdk is not None:
          ##  success_message = 'fMRIPrep finished without errors'
            success_message = 'MEPrep finished without errors'   ##ZSW
