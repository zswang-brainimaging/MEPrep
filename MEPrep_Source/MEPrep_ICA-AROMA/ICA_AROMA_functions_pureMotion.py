...
def classification(outDir, maxRPcorr, edgeFract, HFC, csfFract):
    ...
    maxRPcorr_threshold = max(maxRPcorr)*0.9
   # motionICs_full = np.squeeze(np.array(np.where((proj > 0) + (csfFract > thr_csf) + (HFC > thr_HFC))))
    motionICs = np.squeeze(np.array(np.where(maxRPcorr > maxRPcorr_threshold)))
