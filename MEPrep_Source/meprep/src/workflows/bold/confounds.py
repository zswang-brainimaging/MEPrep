...
## ZSW
"""
The following function is to implement saving multiple sets of results by modifing desc
str1 = tedena_choice_selected
str2 = the original desc
made by Dr. ZSW Wang
"""
###ZSW
def get_MEPrep_Method(tedana_choice, preDenoising_choice):
    MEPrepMethod_dict = {
        'Raw': {
            'optcom': 'optcom',
            'optcomDenoised': 'meica'
        },
        'preICA': {
            'optcom': 'poptcom', 
            'optcomDenoised': 'pmeica'
        }
    }
    base_preDenoising_choice = preDenoising_choice
    if 'preICA' in preDenoising_choice:
        base_preDenoising_choice = 'preICA'
    MEPrep_Method = MEPrepMethod_dict[base_preDenoising_choice][tedana_choice]
    return MEPrep_Method
    
## ZSW

####ZSW 8/2022, 8/2023
def combine_string(str1, str2, multiecho):
    if not multiecho:
        str12 = str2
    else:
        if str2=="":
