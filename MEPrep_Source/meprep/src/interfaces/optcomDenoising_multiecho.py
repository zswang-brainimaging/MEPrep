...
class run_tedana(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        if tedana_choice == "echo2" and not Output_Fitting_Data:
          ...
          #  self._results['T2starmap'] = "Echo2_NoT2starmap.nii.gz"
          #  self._results['S0map'] = "Echo2_NoS0map.nii.gz"
          #  self._results['Residualmap'] = "Echo2_NoResidualmap.nii.gz"
          #  self._results['Rsquaremap'] = "Echo2_NoRsquaremap.nii.gz"
        ...
        with threadpool_limits(limits=n_threads, user_api=None):
           if tedana_choice =="optcomDenoised":
              ...
              self._results['optcomDenoised_bold'] = fullfile(out_dir, "desc-optcomDenoised_bold.nii.gz")
              ...
              self._results['tedana_output_bold'] = fullfile(out_dir, "desc-optcomDenoised_bold.nii.gz")
              self._results['tedana_choice_selected'] = tedana_choice
...
def output_model_fitting_data(model_fitting_data, output_dir, source_file, output_entities, write_data_method):
     if write_data_method == "Use_DerivativesDataSink":
         ...
         inputnode = pe.Node(niu.IdentityInterface(fields=['source_file', 'T2starmap', 'S0map', 
                                                           'Residualmap', 'Rsquaremap']), 
                             name='inputnode')
         ...
         inputnode.inputs.Residualmap = model_fitting_data['S0map']
