...
def _build_parser(**kwargs):
    ...
    # verstr = f'fMRIPrep v{config.environment.version}'  ##ZSW
    
    verstr = f"MEPrep v{config.environment.version}"    ###ZSW
    
    currentv = Version(config.environment.version)
    is_release = not any((currentv.is_devrelease, currentv.is_prerelease, currentv.is_postrelease))

    parser = ArgumentParser(
      ##  description=f'fMRIPrep: fMRI PREProcessing workflows v{config.environment.version}',  ##ZSW
        description=f'MEPrep: ME-fMRI PREProcessing workflows v{config.environment.version}', ###ZSW
...
https://fmriprep.readthedocs.io/en/%s/spaces.html"""
    ...
    #### WZS
        ### ZSW
        ### Wang
        #### Adding MEfMRIPrep, MEPrep option lines on 8/19/2022
      # tedana options, added by Zhishun Wang on Dec 16, 2021, Aug 2022, Aug 2023
    g_tedana = parser.add_argument_group("tedana multi-echo BOLD preprocessing options")  
    g_tedana.add_argument(
              ...
              dest="tedana_choice",
              default="optcom",
              choices=("optcom", "optcomDenoised", "Both"),
              type=str,
              help="The choices for preprocessing multi-echo BOLD data. "
              ...
              "Both: Run and output both results from optcom and optcomDenoised workflows"
              "(default is optcom, which is the default and only option in the original fmriprep pipeline.)",
        )
          
      # preICA options, added by ZSW
    g_preICA = parser.add_argument_group("preICA to pre-dnoise multi-echo BOLD options")  
    g_preICA.add_argument(
              "--preDenoising-choice",
              "--preDenoising_choice",
              nargs="?",
              dest="preDenoising_choice",
              default="Raw",
              choices=("Raw", "preICA_scme", "preICA_se", "preICA_tensor", "preICA_tcme"),
              type=str,
              help="The choices for preprocessing multi-echo BOLD data with ICA to denoise. "
              "Raw: Not running preICA on multi-echo data (default), but usig raw data after fMRIPrep processed instead;  "
              "preICA_se: use ICA to denoise each echo data before calling TEDANA (a pipeline to process multi-echo data)" 
              "by which noise such as motion artefacts can be removed to improve T2* model fitting;  "
              "preICA_tensor: use tensor-ICA to denoise multi-echo data before calling TEDANA;  "
              "preICA_scme: use ICA to denoise spatially concatenated multi-echo data before calling TEDANA;  "
              "preICA_tcme: use ICA to denoise temporarily concatenated multi-echo data before calling TEDANA ",
        )
     # echo2 options, added by Zhishun Wang on 2022
    g_echo2 = parser.add_argument_group("Output multi-echo echo 2 data option") 
    g_echo2.add_argument(
              "--output-echo2",
              "--output_echo2",
              action="store_true",
              default=False,
              help="Output echo 2 data to compare with the optimally combined and/or denoised optimally combined multi-echo data",
          )
    #### END of adding the MEfMRIprep option lines on 8/19/2022, 3/2024, 5/2025
        #### WZS
        ##ZSW
    ###### MEPrep 
...
applied."""
    ...
    output_layout = "legacy"    ### ZSW
    
    ###ZSW
    preDenoising_choice = config.execution.preDenoising_choice
    meprep_dir = "meprepV2401_output" #+ preDenoising_choice
    ###ZSW
    ...
    if config.execution.fs_subjects_dir is None:
        if output_layout == 'bids':
         ##   config.execution.fs_subjects_dir = output_dir / 'sourcedata' / 'freesurfer'
            config.execution.fs_subjects_dir = output_dir / "freesurfer"   ### ZSW
    ...
    if config.execution.fmriprep_dir is None:
        if output_layout == 'bids':
            ...
            config.execution.fmriprep_dir = output_dir / meprep_dir    ##"meprep"  ##ZSW
        elif output_layout == 'legacy':
